...
<?php  
    ...
    join restaurant r on r.id_restaurant = c.id_restaurant and  p.id_categorie = 'Deserturi' and r.oras = '".$_SESSION['oras']."' order by 1 ASC) ");*/
    ...
    while (($row = oci_fetch_array($sql, OCI_BOTH)) != false) {
        if($aux == 0){
            echo '<div class="casuta rotate_right">
              ...
              <form action="Deserturi.php" method="post">
              <input type="submit" name='.$row['COMBINARI'].' value="Comanda"></form></div>';
            $aux = 1;
        }
        else{
            echo '<div class="casuta rotate_left">
              <p class="prima">'.$row['NUME'].' <br>Restaurant: '.$row['RESTAURANT'].' <br>Pret: '.$row['PRET'].'</p>  
              <form action="Deserturi.php" method="post"><input type="submit" name='.$row['COMBINARI'].' value="Comanda"></form></div>';
            $aux = 0;
        }
